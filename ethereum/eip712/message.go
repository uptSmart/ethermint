...
func createEIP712MessagePayload(data []byte) (eip712MessagePayload, error) {
	...
	if err != nil {
		return eip712MessagePayload{}, errortypes.Wrap(
			err,
			"failed to flatten payload JSON messages",
		)
	}
...
func unmarshalBytesToJSONObject(data []byte) (gjson.Result, error) {
	if !gjson.ValidBytes(data) {
		return gjson.Result{}, errortypes.Wrap(errortypes.ErrJSONUnmarshal, "invalid JSON received")
	}

	payload := gjson.ParseBytes(data)

	if !payload.IsObject() {
		return gjson.Result{}, errortypes.Wrap(
			errortypes.ErrJSONUnmarshal,
			"failed to JSON unmarshal data as object",
		)
	}
...
func getPayloadMessages(payload gjson.Result) ([]gjson.Result, error) {
	...
	if !rawMsgs.Exists() {
		return nil, errortypes.Wrap(
			errortypes.ErrInvalidRequest,
			"no messages found in payload, unable to parse",
		)
	}
	...
	if !rawMsgs.IsArray() {
		return nil, errortypes.Wrap(
			errortypes.ErrInvalidRequest,
			"expected type array of messages, cannot parse",
		)
	}
...
func payloadWithNewMessage(
	...
	if payload.Get(field).Exists() {
		return gjson.Result{}, errortypes.Wrapf(
			errortypes.ErrInvalidRequest,
	...
	if !msg.IsObject() {
		return gjson.Result{}, errortypes.Wrapf(
			errortypes.ErrInvalidRequest,
			"msg at index %d is not valid JSON: %v",
			index,
			msg,
		)
	}
